...
public class SpriteCopy : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        GUILayout.Space(40f);
        GUILayout.BeginHorizontal();
        GUILayout.Label("Copy to path:", EditorStyles.boldLabel);
        copyPath = EditorGUILayout.TextField(copyPath, GUILayout.Width(400));
        GUILayout.EndHorizontal();

        GUILayout.Space(10f);
        if (GUILayout.Button("Copy slices to all"))
        {
            CopyToAll();
        }
    }
    ...
    void CopyToAll()
    {
        ...
        string[] assets = AssetDatabase.FindAssets("t:Texture2D", new string[]{copyPath});
        for (int j = 0; j < assets.Length; j++) {
            ...
            string copyToPath = AssetDatabase.GUIDToAssetPath(assets[j]);
